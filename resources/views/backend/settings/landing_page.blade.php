                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" name="psection1" required> 
                                    ...
                                    @foreach($country_list as $country)
                                        <option value="{{ $country->id }}" {{ $country->id == App\Models\Settings::where('name', 'landing_page_psection_1')->first()->$country->id ? "selected" : "" }}>{{ $country->country_name }}</option>
                                    @endforeach                                                                 
                                </select>
                            </div> 
                        </div>
                        <div class="col-6">
                            <div class="form-group">
                                ...
                                <select class="form-control" name="psection2" required>
                                        <option disabled selected value="">Select..</option> 
                                    @foreach($country_list as $country)
                                        <option value="{{ $country->id }}" {{ $country->id == App\Models\Settings::where('name', 'landing_page_psection_2')->first()->$country->id ? "selected" : "" }}>{{ $country->country_name }}</option>
                                    @endforeach                                                                    
                                </select>
                            </div> 
                        </div>
